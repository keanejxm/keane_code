...
class SpiderSchedulerWeMedia(SpiderScheduler):
    """
    调度器。
    """

    def __init__(self, logger):
        super(SpiderSchedulerWeMedia, self).__init__(logger=logger)
        self._api_params["spider_type"] = "forum"
    ...
    def perform(self):
        ...
        try:
            for i, hits in enumerate(
                    scroll_search_es(self._es_conn, index_name, query_body, per_hits=1, limit=1 * 10 ** 7, scroll="1h"),
                    start=1
            ):
...
def run_scheduler():
    ...
    SpiderSchedulerWeMedia(logger).perform()
