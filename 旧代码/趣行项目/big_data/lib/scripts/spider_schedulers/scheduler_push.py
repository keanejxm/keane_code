...
class SpiderSchedulerPush(SpiderScheduler):
    """
    调度器。
    """

    def __init__(self, logger):
        super(SpiderSchedulerPush, self).__init__(logger=logger)
        # self._api_params["spider_type"] = "push"
    ...
    def perform(self):
        ...
        query_body = {
            "_source": ["platformName","platformID"],
            "query": {"bool": {"must": [
                {"term": {"targetType": 2}},
            ]}}
        }
        ...
        try:
            for i, hits in enumerate(
                ...
                try:
                    ...
                    api_url = f"{api_host}/dc/spiders/run/push/"
                    resp = requests.post(api_url, params=self._api_params, json=post_body, timeout=self.timeout)
...
def run_scheduler():
    ...
    SpiderSchedulerPush(logger).perform()
