...
class SpiderSchedulerEPaper(SpiderScheduler):
    ...
    def __init__(self, logger):
        ...
        self._api_params["spider_type"] = "epaper"
        self._api_url = "http://127.0.0.1:17000/dc/spiders/run/"
    ...
    def perform(self):
        ...
        # 执行更新。
        try:
            for i, hits in enumerate(result, start=1):
                # noinspection PyBroadException
                try:
                    post_body = dict(
                        targetDates=[yestoday, today, ],
                        template=hits,
                    )
