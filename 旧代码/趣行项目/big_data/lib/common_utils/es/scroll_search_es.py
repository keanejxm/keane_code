...
def scroll_search_es(es_conn, index_name, query_body=None, per_hits=100, limit=1, scroll="5m"):
    """
    从ES中查询数据。
    :return:
    """
    ...
    try:
        ...
        if length < limit:
            for i in range(0, int(total / per_hits) + 1):
                try:
                    scroll_response = es_conn.scroll(scroll_id=scroll_id, scroll=scroll)
                    for scroll_hit in scroll_response['hits']['hits']:
                        scroll_hit['_source']['_id'] = scroll_hit['_id']
                        yield scroll_hit['_source']
