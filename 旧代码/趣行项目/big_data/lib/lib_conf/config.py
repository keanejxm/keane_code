...
lib_path = f"{BASE_DIR}/lib"
log_path = f"{BASE_DIR}/logs"

# 特殊路径配置。
if env.run_env == env.production_env:
    # 其它路径。
    api_log_path = f"/opt/data/{project_name}/logs"
else:
    pass
...
# 上传文件至BOS接口。
bos_upload_by_url_api = f"{api_host}/dc/cloud_storage/bos/upload_by_url"
# 上传图片至BOS并且计算图片哈希值接口。
bos_upload_image_by_url_and_compute_hash_api = f"{api_host}/dc/cloud_storage/bos/upload_image_by_url_and_compute_hash"
