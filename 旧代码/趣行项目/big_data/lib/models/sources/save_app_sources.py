...
class TempSources(SourceData):
    ...
    def save_to_es(self, path):
        ...
        for item in content.values():
            ...
            if "is_main" in item and item["is_main"] == 1:
                # 主体，可能是报纸名、网站名、账号名等。
                res1 = re.findall(r"^4_1_\d*_\d*$",field_id)
                res2 = re.findall(r"^4_4_\d*$|^4_3_\d*$",field_id)
                res3 = re.findall(r"^4_2_\d*_\d*$",field_id)
                res4 = re.findall(r"^4_5_\d*_\d*$",field_id)
                if res1:
                    print(res1,item["name"])
                if res2:
                    print(res2,item["name"])
                if res3:
                    print(res3,item["name"])
                if res4:
                    print(res4,item["name"])
    ...
    def save(self):
        ...
        :return:
        """
        paths = rf"/home/debugger/neil/big_data/big_data_platform/lib/models/sources/app/APP数据.json"
