...
class WebSpider(object):
    ...
    def parse_doc_page(self, doc_link, channel_name):
        ...
        fields["platformType"] = self._platform_type
        fields["channelID"] = ""
        fields["channelName"] = ""
        fields["url"] = url
        # fields["html"] = _html
        ...
        # 为首页轮播新闻,头条新闻,添加标签
        # 3_3_1国家机构,其余为省级市级政府网站，为政府头条添加判断逻辑
        if channel_name == "首页":
            if "3_3_1" in self._parents_id or "3_2_2_2" in self._parents_id \
                    or "3_2_3_1" in self._parents_id or "3_2_4_2" in self._parents_id or "3_2_6_2" in self._parents_id \
                    or "3_2_5_3" in self._parents_id or "3_2_7_3" in self._parents_id \
                    or "3_2_8_2" in self._parents_id or "3_2_9_2" in self._parents_id \
                    or "3_2_10_2" in self._parents_id or "3_2_11_2" in self._parents_id \
                    or "3_2_12_2" in self._parents_id:
                fields["classifications"] = ["rcmd_2"]
            # 网媒头条
            elif "3_3" in self._parents_id or "3_2_1_3" in self._parents_id or "3_2_2_3" in self._parents_id \
                    or "3_2_3_2" in self._parents_id or "3_2_4_3" in self._parents_id \
                    or "3_2_5_4" in self._parents_id or "3_2_6_3" in self._parents_id \
                    or "3_2_7_4" in self._parents_id or "3_2_8_3" in self._parents_id \
                    or "3_2_9_3" in self._parents_id or "3_2_10_3" in self._parents_id \
                    or "3_2_11_3" in self._parents_id or "3_2_12_3" in self._parents_id:
                fields["classifications"] = ["rcmd_3"]
            else:
                ...
                for channel_info_xpath in self._channel_info_xpaths:
                    ...
                    for channel_info in tree.xpath(channel_info_xpath):
                        try:
                            ...
                            elif channel_info.xpath("./strong//text()"):
                                channel_names = channel_info.xpath("./strong//text()")
                            else:
                                pass
                            channel_name = "".join(
    ...
    def fetch_yield(self):
        ...
        :return:
        """
        self._logger.info(f"{self._paper['platformName']}开始采集,流数据")
                ...
                try:
                    ...
                    for detail_url in detail_urls:
                        try:
                            ...
                            # 返回文章信息
                            yield dict(code=1, msg="ok", data=dict(works=result))
                            count+=1
                        except Exception as e:
                            self._logger.warning("Failed to parse_doc_page, '{}'.".format(e))
                            # continue
                    # 如果该频道下数据为0，直接进行下一个频道采集。
                    if count == 0:
                        continue
                    # 返回频道信息
                    yield dict(code=1, msg="ok", data=dict(channel=channel_row))
                except Exception as e:
    ...
    def fetch_batch(self):
        ...
        :return:
        """
        self._logger.info(f"{self._paper['platformName']}开始采集,批数据")
        result_data = list()
        start_url = self._start_url
        # noinspection PyBroadException
        try:
            ...
            for channel_row in self.fetch_channels(start_url):
                ...
                # 添加频道结果
                base_item["channels"].append(channel_row)
                try:
                    channel_url = channel_row.get("url")
                    channel_name = channel_row.get("name")
                    # 获取每一个频道新闻详情列表
                    ...
                    for detail_url in detail_urls:
                        try:
                            ...
                            result["channelName"] = channel_row.get("name", "")
                            # 添加文章结果
                            base_item["worksList"].append(result)
                        except Exception as e:
                            self._logger.warning("Failed to parse_doc_page, '{}'.".format(e))
                            continue
                except Exception as e:
                    self._logger.warning("Failed to fetch channel, '{}'.".format(e))
                    continue
                result_data.append(base_item)
            return {"code": 1, "msg": "OK", "data": result_data}
        except Exception as e:
            self._logger.warning("Failed to start crawler, '{}'.".format(e))
