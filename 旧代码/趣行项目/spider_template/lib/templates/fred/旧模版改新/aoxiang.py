...
class Aoxiang(Appspider):
    ...
    def getarticlelistsparams(channelsparams):
        ...
        for channel in channelsparams:
            ...
            if channelname == "政务":
                ...
                method = "post"
                articlelist_param = InitClass().articlelists_params_fields(url, headers, method, channelname,channelid = channelid,channeltype=channeltype, data=data,
            ...
            articlelist_param = InitClass().articlelists_params_fields(url, headers, method, channelname,channelid = channelid,channeltype=channeltype, data = data,channeljson=channeljson)
    ...
    def getarticleparams(articles):
        ...
        for articleparam in articles:
            ...
            videos = specialid #articleparam.get("videos")
            ...
            likenum = channelid#articleparam.get("likenum")
            commentnum = articleparam.get("commentnum")
            sharenum = articleparam.get("sharenum")
            readnum = articleid#articleparam.get("readnum")
            # articleurl = articleparam.get("articleurl")
    ...
    def analyzearticles(articles_res):
        for articleres in articles_res:
            ...
            #
            specialid = articleres.get("videourl") #当前专题列表id
            ...
            articletitle = articleres.get("videocover")
            ...
            try:
                ...
                intro = articlejson["introduce"]
                topic_fields["digest"] = intro

                contentlist = articlejson['list']
                ...
                for article in contentlist:
                    ...
                    aid = article["newsId"]
                    aurl = article["url"]
                    asource = article["infoSource"]
                    atime = article["time"]
                    atime = InitClass().date_time_stamp(atime)

                    if atime > newestpubtime:
                        newestpubtime = atime
                        newestarticleid = aid

                    acovers = article["imgUrls"]
                    ...
                    if newstype == 1 or newstype == 2:
                        # 文字
                        name = tree.xpath('//div[@class="detail"]')
                        article_fields["contentType"] = 1
                    elif newstype == 8:
                        # 画廊
                        name = tree.xpath('//div[@class="detailLists"]')
                        article_fields["contentType"] = 6
                    else:
                        name = tree.xpath('//div[@class="detail"]')
                        # print("article==",article)
                        article_fields["contentType"] = 1
