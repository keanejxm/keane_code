...
class Appspider(object):
    ...
    def getchannel(self, appparams):
        ...
        for appparam in appparams:
            ...
            if method == "get":
                if appjsons:
                    channelres = self._session.get(url, headers = headers, params = data,
                                                  json = appjsons).content.decode()
                else:
                    channelres = self._session.get(url, headers = headers, params = data).content.decode()
                channelsres.append(channelres)
            if method == "post":
                channelres = self._session.post(url, headers = headers, data = data, json = appjsons).content.decode()
                channelsres.append(channelres)
        yield channelsres