...
class AppSpideraShiJiaZhuangRiBao(object):
    ...
    def wash(content, site_url):
        ...
        for x in soup.find_all('a'):
            ...
            if 'href' in label:
                string = x.get_text()
                strint_list.append(string)
        info = []
        for s in soup('a'):
            info.append(s)
        for i, ind in enumerate(strint_list):
            index = int(i)
            t = info[index]
            content = content.replace(str(t), ind)

        # 去除无用属性style，width，height，合并相对链接的img标签
        content_soup = bs4.BeautifulSoup(content, 'html.parser')
        for x in content_soup.find_all():
            if isinstance(x, bs4.Tag):
                if x.name == "video":
                    # self._print(x.attrs)
                    if not x.has_attr("controls"):
                        x.attrs["controls"] = "controls"
                        # self._print(x.attrs)
            del x["style"]
            del x["width"]
            del x["height"]
        ...
        # 图片链接替换为绝对路径。
        for s in content_soup('img'):
            if "src" not in s.attrs:
                if "data-himg" in s.attrs:
                    s['src'] = urljoin(site_url, s.attrs['data-himg'])
                elif "data-original" in s.attrs:
                    s['src'] = urljoin(site_url, s.attrs['data-original'])
    ...
    def article(self, site_url):
        ...
        # 发布时间。
        pub_time_str = html.xpath("//span[@class='media-date'][1]/text()")[0]
        pub_time = int(time.mktime(time.strptime(pub_time_str, "%Y-%m-%d %H:%M")))
        # 正文。
        content = html.xpath("//div[@id='aritcleContent']")
        content = list(map(lambda x: etree.tostring(x, encoding="unicode").strip(), content))[0]
        content = self.wash(content, site_url)
        # 正文图。
    ...
    def generate_fields(self, site_url, response, extend_fields):
        ...
        for item in data:
            try:
                ...
                if "nid" not in item or item["nid"] in (0, "0"):
                ...
                if "publish_time" in item and item["publish_time"]:
                    ...
                    if "update_time" in item and item["update_time"]:
                        fields['pubTime'] = int(time.mktime(time.strptime(str(item["update_time"]), "%Y-%m-%d %H:%M:%S")))
                    else:
                        continue
                ...
                filler = dict()
                # 转载来源。
                if "copyfrom" in item and item['copyfrom']:
                    filler['pubSource'] = item['copyfrom']
                else:
                    filler['pubSource'] = ""
                fields['filler'] = json.dumps(filler, separators=(",", ":"))
                # 详情页连接。
                url = "http://static.sjzrbapp.com:89/news/newsinfo"
                if "nid" in item and item["nid"] not in ("0", 0):
                    # 规则一。
                    body = {
                        "nid": str(item["nid"]),
                    ...
                    try:
                        ...
                        if detail_response.status_code == requests.codes.ok:
                            ...
                            # self._print(detaile_article)
                            fields["url"] = detaile_article["data"]["shareUrl"]
                            # 赋值正文。
                            if "link" in detaile_article["data"] and detaile_article["data"]["link"]:
                                fields['content'] = detaile_article["data"]["link"]
                            elif "video_path" in detaile_article["data"] and detaile_article["data"]["video_path"]:
                                fields['content'] = detaile_article["data"]["video_path"]
                            else:
                                pub_time, content = self.article(fields["url"])
                ...
                # 正文是否有图。
                if "<img " in fields["content"]:
                    fields['hasPic'] = 1
                else:
                    fields['hasPic'] = 0

                # 正文是否有视频。
                if "<video " in fields["content"]:
                    fields['hasVideo'] = 1
                else:
                    fields['hasVideo'] = 0
    ...
    def deal_with_list_response(self, rule, channel_name):
        ...
        if response.status_code == requests.codes.ok:
            ...
            for field_id, fields in self.generate_fields(url, response.text,
                ...
                # self._print(field_id)
                self._print(json.dumps(fields, indent=4, ensure_ascii=False))
...
def run():
    # 执行。
    try:
        AppSpideraShiJiaZhuangRiBao().run()
    except Exception as e:
